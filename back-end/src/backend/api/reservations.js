...
router.get("/", async (req, res) => {
  try {
    ...
    if (allReservations) {
      ...
      res.status(404).send("No reservations founded");
    }
  } catch (error) {
    console.error("Server Error", err);
    res.status(500).json({ error: "Server Error: Not able to import data" });
  }
});
...
router.post("/", async (req, res) => {
  try {
    ...
    if (newReservationAdded) {
      res
        .status(201)
        .json({ message: "The reservation has created successfully" });
    }
    res.status(404).send("The data you have requested is not found");
  } catch (err) {
    console.error("Server Error", err);
    res.status(500).send({ error: "Server Error: Not able to import data" });
  }
});
...
router.put("/:id", async (req, res) => {
  try {
    ...
    if (updatedReservationById) {
      ...
      res.status(404).send("The reservation is not found");
    }
  } catch (error) {
    res.status(500).json({ error: "Server Error: Not able to import data" });
  }
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    if (deletedReservationById) {
      res.json({ message: "The reservation has deleted successfully" });
    } else {
      res.status(404).json({ error: "The reservation is not found" });
    }
  } catch (error) {
    res.status(500).json({ error: "Server Error: Not able to import data" });
  }
